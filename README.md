...
当前主线前端已经升级为浅色主导的 Apple 风格研究工作台，首页、股票列表、行业看板、个股详情、历史运行、任务中心、Run 复盘和规则实验台都使用统一的轻卡片系统、克制标题和摘要优先的信息结构。

其中，规则实验台已经重构为左侧编辑、右侧反馈的实验驾驶舱，支持改动摘要、实时校验、恢复默认，以及“仅本次生效并运行 / 保存为默认规则并运行”两条清晰的提交路径。
...
启动脚本会优先尝试前端默认端口 `4178`。如果该端口被占用，Vite 会自动切到其他端口，例如 `4179`；此时请以 `data/dev-console/frontend.log` 里的 `Local` 地址为准，而不是只看脚本默认提示。
